...
# Save as RDS
saveRDS(pcr_data_expanded, here("data", "processed-data", "rds", "pcr_data"))

# Save as CSV
write_csv(pcr_data_expanded, here("data", "processed-data", "pcr_data.csv"))
...
## Bird data
#### This dataset comes from the annual North American Breeding Bird Survey that is conducted in a joint effort by: USGS Eastern Ecological Science Center, Environment and Climate Change Canada, and the Mexican National Commission for the Knowledge and Use of Biodiversity.
#### There are 2 (5 and 11) routes in Bristol County, MA. The data is from 2007-2022. However, route 11 is mission data for 2013 and route 5 ends in 2019.
#### For route 11, 2013, we will use the data from 2012 and 2014 to estimate the missing data.
for route 5, 2020-2022, we will use the data from 2019 to estimate the missing data.
```{r}
# Load the dataset
file_path_bird <- here("data", "raw-data", "Massach.csv")
bird_data_raw <- read.csv(file_path_bird)

# Filter and clean up the data
bird_bbs <- bird_data_raw %>%
  filter(Route %in% c(5, 11), Year >= 2006 & Year <= 2022) %>%
  select(-AOU, -Count10, -Count20, -Count30, -Count40, -Count50)

# Summarize the data
bird_bbs_summarized <- bird_bbs %>%
  group_by(Route, Year) %>%
  summarize(
    StopTotal = sum(StopTotal, na.rm = TRUE),
    SpeciesTotal = sum(SpeciesTotal, na.rm = TRUE),
    .groups = 'drop'
  )

# Check for missing data within Routes 5 and 11
missing_data <- map_df(c(5, 11), ~tibble(
  Route = .x,
  missing_years = setdiff(2006:2022, bird_bbs_summarized$Year[bird_bbs_summarized$Route == .x])
))

# Identify all combinations of Route and Year within the range
all_combinations <- expand.grid(Route = unique(bird_bbs_summarized$Route),
                                Year = 2006:2022)

# Join with the summarized data to identify missing rows
bird_bbs_full <- left_join(all_combinations, bird_bbs_summarized, by = c("Route", "Year"))

# Impute missing data by taking the average of the previous and next year for each route
bird_bbs_imputed <- bird_bbs_full %>%
  group_by(Route) %>%
  arrange(Route, Year) %>%
  mutate(
    StopTotal = ifelse(is.na(StopTotal), (lag(StopTotal) + lead(StopTotal)) / 2, StopTotal),
    SpeciesTotal = ifelse(is.na(SpeciesTotal), (lag(SpeciesTotal) + lead(SpeciesTotal)) / 2, SpeciesTotal)
  ) %>%
  ungroup()

# Handling the last records for Route 5 (2020-2022)
# Since there's no future data to average, you might replicate the most recent data (e.g., from 2019)
# Or consider another imputation method suitable for your analysis and data characteristics

# Find the most recent year's data for Route 5
recent_data_route_5 <- bird_bbs_imputed %>% 
  filter(Route == 5, Year == max(Year[Route == 5 & !is.na(StopTotal)]))

# Create a template for the missing years
missing_years_route_5 <- tibble(Route = 5, Year = 2020:2022)

# Replicate the most recent data for the missing years
imputed_missing_years_route_5 <- bind_rows(lapply(missing_years_route_5$Year, function(y) {
  mutate(recent_data_route_5, Year = y)
})) %>%
  bind_rows(bird_bbs_imputed) %>%
  arrange(Route, Year)

# Check if any adjustments or specific imputations are needed
print(imputed_missing_years_route_5)

bird_clean <- imputed_missing_years_route_5 %>%
  drop_na()

# Save the cleaned bird data as an csv
write_csv(bird_clean, here("data", "processed-data", "bird_clean.csv"))
```
...
# Since mosquito populations can be affected by the previous year's populations, we will filter for 2007-2023
...
View(mosquito_wide_filtered.csv)
# Define the path for the RDS file
...
proportion_mel_by_year <- trap_proportion %>%
  ...
  ungroup() %>%
  # Then group by year and summarize
  group_by(Year) %>%
...
proportion_mel_by_week <- trap_proportion %>%
  # Calculate row-wise totals for all species
  rowwise() %>%
  mutate(Total_All = sum(c_across(ABS:OCH), na.rm = TRUE)) %>%
  ungroup() %>%
