...
library(readr)         # for reading & writing rectangular data in text format
...
### Load the dataset, define predictors, and prepare the data for modeling.
```{r}
```{r}
# Load the dataset
mir_1part_all <- read_csv(here("data", "processed-data", "all_mir_clean_joined.csv"))
...
```
### Define the RF model specifications, create recipe, and setup workflow
```{r}
...
## Tune the model, extract the best hyperparameters, and fit the final model
```{r}
```{r}
...
### Extract the fitted model, calculate feature importance
```{r}
...
### Bootstrapping to evaluate model robustness
```{r}
...
# Define tuning grid for LASSO with a wide range of penalty values
tuning_grid_lasso <- expand.grid(
  penalty = 10^seq(-10, 1, length.out = 50)  
...
# Tune the model using the tuning grid and folds
...
### Extract the fitted model and calculate RMSE for the training and testing sets
```{r}
...
### Additionally, LASSO models are not ideal for capturing non-linear relationships.
...
### Gradient Boosting Machine (GBM) is an ensemble learning technique that builds models sequentially, with each new model correcting errors made by the previous ones. It uses decision trees as the base learners and optimizes a loss function, making it effective for both regression and classification tasks.
```{r}
# Load the dataset
mir_1part_all <- read_csv(here("data", "processed-data", "all_mir_clean_joined.csv"))
...
### Bootstrap to evaluate GBM robustness
```{r}
